<#
    ...
    What this does:
      ...
      2. Downloads the matching prebuilt "akk.exe" from the latest
         GitHub release.
      3. Installs it to $AkkInstallDir (default: %LOCALAPPDATA%\Akkhara\bin).
    ...
    Env vars you can override before piping into iex:
        $env:AKK_REPO        "owner/repo"      (default: yourusername/akkhara)
        $env:AKK_VERSION     a release tag      (default: latest)
        $env:AKK_INSTALL_DIR install directory  (default: %LOCALAPPDATA%\Akkhara\bin)

    NOTE: This script downloads a prebuilt binary. If you want to build
    from source instead (e.g. you're developing Akkhara itself), use the
    project's own install.ps1 / command.ps1 in the repo instead of this one.
#>
...
function Write-Fail { param([string]$Text) Write-Host "    [FAILED] $Text" -ForegroundColor Red; exit 1 }
...
switch ($arch) {
    ...
    "ARM64" { Write-Fail "arm64 Windows builds aren't published yet. Build from source instead: see the project's README." }
    default { Write-Fail "unsupported architecture: $arch" }
}
Write-Ok "Detected $target"

# ---------------------------------------------------------------------
# 2. Resolve download URL from the GitHub Releases API
# ---------------------------------------------------------------------
...
if ($userPath -split ";" | Where-Object { $_ -eq $InstallDir }) {
    ...
    $env:Path += ";$InstallDir"   # make it available in this session too
