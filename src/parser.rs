...
pub enum Expr {
    ...
    /// HTTP GET via a network-capable native library: `<lib> ၏ <url>`,
    /// e.g. `request ၏ "https://api.example.com/data"`. `<lib>` must name
    /// a recognized library (currently only "request"); the interpreter
    /// checks that it has actually been imported before calling out.
    HttpGet(String, Box<Expr>, usize),
}
...
pub enum Stmt {
    ...
    /// HTTP send (POST-style): `<var> အတွက် <lib> ၏ <url> သို့ <data> ကို
    /// ပို့ပါ။` -- sends `<data>` to `<url>` via `<lib>` (currently only
    /// "request" is supported) and stores the resulting response object
    /// into `<var>`.
    HttpSend {
        name: String,
        lib: String,
...
// Member access / network library keywords.
const KW_OF: &str = "၏";
const KW_SEND: &str = "ပို့ပါ";
...
fn parse_term_at(tokens: &[Token], pos: usize, line: usize) -> Result<(Expr, usize), String> {
    ...
    loop {
        ...
        //                                `request ၏ "https://..."`.
        //   <expr> ၏ <field ident>    -- generic field access on an
        //                                object value, e.g.
        //                                `response ၏ အခြေအနေကုဒ်`.
        if i < tokens.len() && ident_eq(&tokens[i].tok, KW_OF) {
            ...
            if i + 1 < tokens.len() {
                if let Tok::Ident(field_name) = &tokens[i + 1].tok {
                    expr = Expr::FieldAccess(Box::new(expr), field_name.clone(), of_line);
                    i += 2;
                    continue;
                }
            }
...
fn parse_stmt(tokens: &[Token], line: usize) -> Result<Stmt, String> {
    ...
    // --- HTTP send (POST-style):
    //     <var> အတွက် <lib> ၏ <url> သို့ <data> ကို ပို့ပါ။
    // e.g. y အတွက် request ၏ "https://api.example.com/submit" သို့ "a=1&b=2" ကို ပို့ပါ။
    if let Some(send_idx) = find_kw(&body, KW_SEND) {
        if send_idx == 0 || !ident_eq(&body[send_idx - 1].tok, KW_PARTICLE) {
            return Err(generic_syntax_err(line));
        }
        let head = &body[..send_idx - 1];
        if head.is_empty() || !matches!(head[0].tok, Tok::Ident(_)) {
            return Err(generic_syntax_err(line));
        }
        let var_name = match &head[0].tok {
            Tok::Ident(s) => s.clone(),
            _ => unreachable!(),
        };
        let for_idx = find_kw(head, KW_FOR).ok_or_else(|| generic_syntax_err(line))?;
        if for_idx != 1 {
            return Err(generic_syntax_err(line));
        }
        let rest = &head[for_idx + 1..];
        if rest.is_empty() || !matches!(rest[0].tok, Tok::Ident(_)) {
            return Err(generic_syntax_err(line));
        }
        let lib_name = match &rest[0].tok {
            Tok::Ident(s) => s.clone(),
            _ => unreachable!(),
        };
        if rest.len() < 2 || !ident_eq(&rest[1].tok, KW_OF) {
            return Err(generic_syntax_err(line));
        }
        let after_of = &rest[2..];
        let to_idx =
            find_kw_top_level(after_of, KW_TO).ok_or_else(|| generic_syntax_err(line))?;
        let url_tokens = &after_of[..to_idx];
        let data_tokens = &after_of[to_idx + 1..];
        if url_tokens.is_empty() || data_tokens.is_empty() {
            return Err(generic_syntax_err(line));
        }
        if !end_present {
            return Err(missing_period_err(line));
